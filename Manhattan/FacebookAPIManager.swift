...
class FacebookAPIManager {
    ...
    func requestFacebookUser(completion: @escaping (_ facebookUser: User) -> Void) {
        let graphRequest = GraphRequest(graphPath: kGraphPathMe, parameters: ["fields":"id,email,last_name,first_name,picture,name,birthday"], accessToken: accessToken, httpMethod: .GET, apiVersion: .defaultVersion)
